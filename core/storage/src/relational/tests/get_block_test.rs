...
async fn test_get_block_by_number() {
    ...
    // from built block view
    let count = pool.block_count().await.unwrap();
...
async fn test_get_block_by_hash() {
    ...
    // get tip
    let res = pool.get_block(None, None).await.unwrap();
    let block_built_hash: H256 = res.hash().unpack();
    assert_eq!(
    ...
    let res_1 = pool
        .get_block(Some(block_number_9_hash.clone()), Some(9))
        .await;
    let res_2 = pool.get_block(Some(block_number_9_hash), Some(1)).await;
    assert!(res_1.is_ok());
    assert!(res_2.is_err());
}
...
async fn test_get_canonical_block_hash() {
    let pool = connect_and_insert_blocks().await;
    let res = pool.get_canonical_block_hash(0).await.unwrap();
    assert_eq!(
