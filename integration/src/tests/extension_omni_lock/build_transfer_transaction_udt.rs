...
inventory::submit!(IntegrationTest {
    name: "test_omni_transfer_udt_to_provide_capacity",
    test_fn: test_omni_transfer_udt_to_provide_capacity
});
fn test_omni_transfer_udt_to_provide_capacity() {
    // issue udt
    issue_udt_1().unwrap();
    let udt_hash = UDT_1_HASH.get().unwrap();
    let acp_address_with_udt = UDT_1_HOLDER_ACP_ADDRESS.get().unwrap();
...
fn test_omni_transfer_udt_from_provide_capacity() {
    ...
    let (omni_identity, omni_address, omni_address_pk) = generate_omni_secp_address_pk_pair();
    let omni_account_address = build_omni_acp_account_address(&omni_address).unwrap();
    let _out_point = prepare_ckb_capacity(&omni_address, 205_0000_0000).unwrap();
    ...
    assert_eq!(146_0000_0000u128, response.balances[0].occupied.into());
    assert_eq!(100u128, response.balances[1].free.into());
    assert_eq!(205_0000_0000u128, response.balances[2].free.into());
    ...
    assert_eq!(146_0000_0000u128, response.balances[0].occupied.into());
    assert_eq!(50u128, response.balances[1].free.into());
    assert!(63_0000_0000u128 < response.balances[2].free.into());
}
