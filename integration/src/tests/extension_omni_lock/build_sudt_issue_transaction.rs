...
use crate::utils::address::get_udt_hash_by_owner;
use crate::utils::address::omni_lock::{
    build_omni_acp_account_address, new_identity_from_omni_address,
    prepare_omni_secp_address_with_capacity,
};
use crate::utils::address::secp::generate_rand_secp_address_pk_pair;
use crate::utils::instruction::{prepare_account, send_transaction_to_ckb};
...
inventory::submit!(IntegrationTest {
    name: "test_omni_issue_udt_from_multi_item",
    test_fn: test_omni_issue_udt_from_multi_item
});
fn test_omni_issue_udt_from_multi_item() {
    // prepare from items
    let (_, owner_address, owner_pk, _) =
        prepare_omni_secp_address_with_capacity(145_0000_0000).expect("prepare ckb");
    let (_, _from_address_1, from_address_pk_1, out_point_1) =
        prepare_omni_secp_address_with_capacity(145_0000_0000).expect("prepare ckb");
    let (_, from_address_2, from_address_pk_2, _) =
        prepare_omni_secp_address_with_capacity(250_0000_0000).expect("prepare ckb");
    let pks = vec![owner_pk, from_address_pk_1, from_address_pk_2];

    // prepare to items
    let (_, to_address_1, to_address_pk_1, _) =
        prepare_omni_secp_address_with_capacity(150_0000_0000).expect("prepare ckb");
    ...
    let to_identity = new_identity_from_omni_address(&to_address_1.to_string()).unwrap();
    ...
    .unwrap();
    let to_acp_address_1 = build_omni_acp_account_address(&to_address_1).unwrap();
